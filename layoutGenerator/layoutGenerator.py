...
sys.path.append('../')
sys.path.append('../../')
from dbcIterator import DbcParser
from CanMsgLayoutDecoder import CanMsgLayoutDecoder
from layoutAnalyzer import SignalBitFieldSyntaxGenerator
                ...
                for msg in myParser.can_network.nodes['ESCL'].node_tx_msgs.values():
                    ...
                    try:
                        for virtual_msg in CanMsgLayoutDecoder(msg).decode_msg_with_multiplexer().node_tx_msgs.values():
                            ...
                            syntax_generator = SignalBitFieldSyntaxGenerator(decoded_msg)
                            syntax_generator.bit_field_generator(hFile)
                            hFile.write('\n}')
                            s += '\t_c_{0}_msg_type {1};\n'.format(virtual_msg.msg_name, virtual_msg.msg_name)
                            hFile.write('_c_{0}_msg_type;\n'.format(virtual_msg.msg_name))
                            syntax_generator.signal_union_generator(msg, cFile)
                            syntax_generator.signal_access_macro_syntax(over_byte_bound_virtual_signals, hFile)
                        ...
                        syntax_generator.signal_struct_generator(cFile)
                        syntax_generator.signal_access_macro_syntax(over_byte_bound_virtual_signals, hFile)
