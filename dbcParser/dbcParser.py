...
class DbcParser(object):
    ...
    def __init__(self):
        ...
        Constructor
        '''
        self._can_network = CoreModel.CanNetwork()
    ...
    def version_hdl(self, line):
        ...
        try:
            self.can_network.version = reg.group('version')
    ...
    def node_hdl(self, line):
        ...
        try:
            for node_name in reg.split(' '):
                self.can_network.append_nodes(node_name, CoreModel.CanNode(node_name))
        except AttributeError:
    ...
    def msg_hdl(self, line):
        ...
        reg = re.search(self._patten, line)
        try:
            self._current_msg_sender = self.can_network.nodes[reg.group('msg_sender')]
        except KeyError:
            print('[WARINING]: the msg sender {0} is not in the list of node definition'.format(reg.group('msg_sender')))
            self._current_msg_sender = CoreModel.CanNode(reg.group('msg_sender'))
            self.can_network.append_nodes(self._current_msg_sender.node_name, self._current_msg_sender)
        # create a message object
        try:   
            self._current_msg = CoreModel.CanMsg(int(reg.group('msg_id')), reg.group('msg_name'), int(reg.group('length')), msg_sender=self._current_msg_sender)
        except AttributeError:
            print("[WARINING] message definition violation")
        # add this message to parent node's tx message
        self.can_network.nodes[self._current_msg.msg_sender.node_name].append_tx_msg(self._current_msg.msg_name,
                                                                                     self._current_msg)
    ...
    def signal_hdl(self, line):
        ...
        try:
            self._current_signal = CoreModel.CanSignal(reg.group('name'), int(reg.group('start_bit')), int(reg.group('length')), int(reg.group('little_endian')), reg.group('signed'),
                                                 float(reg.group('factor')), float(reg.group('offset')), float(reg.group('value_min')), float(reg.group('value_max')), reg.group('unit'),
                                                 reg.group('multiplexer'), reg.group('multiplexer_id'), msg_carrier=self._current_msg)
        except AttributeError:
    ...
    def iterate_dbc(self, f):
        for node_name, node in self.can_network.nodes.items():
            f.write("{0}\n".format(node_name))
